...
class Trainer(object):
    ...
    def _train_net(self, epoch):
        losses, acc1 = tv.train(self.cfg, epoch, self.algorithm, self.train_loader, self.use_cuda, self.logger)
        return losses, acc1
    
    def _val_net(self, epoch, dataloader):
        loss, top1, accuracies, save_label_list, data_list = tv.val(self.algorithm, dataloader, self.use_cuda, False, self.writer)
        return loss, top1, accuracies, save_label_list, data_list
    ...
    def train(self):
        if self.stage >= 2:
            for epoch_item in range(self.start_epoch, self.epochs):
                ...
                val_loss, val_top1, _, _, _ = self._val_net(epoch_item, self.val_loader)
                test_loss, test_top1, _, _, _ = self._val_net(epoch_item, self.test_loader)
            ...
            val_loss, val_top1, val_accs, val_label_list, val_data_list = self._val_net('val', self.val_loader)
            val_data_pkl = {'data':np.concatenate(val_data_list),'label':val_label_list}
            file_path = str(self.output_path)+'/val_data.pkl'
            with open(file_path, 'wb') as file:
                pickle.dump(val_data_pkl, file)
            test_loss, test_top1, test_accs, test_label_list, test_data_list = self._val_net('test', self.test_loader)
            tes_data_pkl = {'data':np.concatenate(test_data_list),'label':test_label_list}
            file_path = str(self.output_path)+'/test_data.pkl'
            with open(file_path, 'wb') as file:
                pickle.dump(tes_data_pkl, file)
