...
def train(args, epoch, algorithm, minibatches_loader, use_cuda=True, writer=None):
    ...
    for iteration in range(iterations):
        ...
        if epoch == args.epochs-1 and iteration == iterations-1:
            logger = writer
        else:
            logger = None
        if use_cuda:
        ...
        loss, pred, targets = algorithm.update(minibatches, None, logger=logger)
...
def val(algorithm, dataloaders, use_cuda=True, progress_bar=False, writer=None):
    ...
    for env_name, domain_idx, dataloader in dataloaders:
        loss, top1, acc, save_label_list_one_domain, data = val_one_domain(domain_idx, algorithm, dataloader, use_cuda, progress_bar)
        losses.append(loss.avg)
        top1s.append(top1.avg.item())
        accuracies.append(acc)
        save_label_list.extend(save_label_list_one_domain)
        data_list.append(data)
    return losses, top1s, accuracies, save_label_list, data_list
...
def val_one_domain(domain_idx, algorithm, dataloader, use_cuda=True, progress_bar=False):
    ...
    for iteration, episode in episodes:
        with torch.no_grad():
            ...
            ### added
            maxk = max((1,))
            _, pred_label = output.topk(maxk, 1, True, True)
            
            save_label_list_one_domain.extend(pred_label.squeeze(-1).tolist())
    ...
    return losses, top1, accuracies, save_label_list_one_domain, np.concatenate((data_list))